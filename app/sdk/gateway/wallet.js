...
const CRYPTO_CONFIG = path.resolve(__dirname, '../../../orgs/oem-designgrp/crypto-config');
...
function readCertCryptogen(org, user) {
    ...
    var certPath = CRYPTO_CONFIG_PEER_ORGS + "/" + org + "/users/" + user + "@" + org + "/msp/signcerts/" + user + "@" + org + "-cert.pem"
...
function readPrivateKeyCryptogen(org, user) {
    ...
    var pkFolder = CRYPTO_CONFIG_PEER_ORGS + "/" + org + "/users/" + user + "@" + org + "/msp/keystore"
...
function createMSPId(org) {
    // return org.charAt(0).toUpperCase() + org.slice(0) + 'MSP'
    // var orgName = org.slice(0,org.indexOf('.'))
    // var temp = orgName.charAt(0).toUpperCase()
    // return temp + orgName.slice(1) + 'MSP'
