...
async function  invokeChaincode() {
    ...
    var request = {
        ...
        fcn: 'NewAsset',
        args: [reqID,"{\"firstname\": \"Rosesh\", \"lastname\": \"Sarabhai\"}",'200'],
...
async function setupChannel() {
    try {
        // Get the channel by the name from client
        channel = await client.getChannel(CHANNEL_NAME,true)
    }catch(e) {
        console.log("Error setting up channel - ",CHANNEL_NAME )
        process.exit(1)
    }
...
async function setupClient() {
    ...
    // setup the instance
    client = await Client.loadFromConfig(CONNECTION_PROFILE_PATH)
